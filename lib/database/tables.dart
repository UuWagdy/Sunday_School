...
class Areas extends Table {
  ...
  IntColumn get level => integer().withDefault(const Constant(0)).named('Level')();
...
class Coming extends Table {
  ...
  IntColumn get behavior => integer().nullable().withDefault(const Constant(5)).named('Behavior')();
}
...
class Pass extends Table {
  ...
  BoolColumn get canMaintenance => boolean().nullable().named('can_maintenance')();
  ...
  BoolColumn get isAdvanced => boolean().withDefault(const Constant(false)).named('is_advanced')();
}

class PersonServices extends Table {
  @override
  String get tableName => 'Person_Services';
  IntColumn get personId => integer().named('Person_ID').references(Persons, #personId, onDelete: KeyAction.cascade)();
  IntColumn get serviceId => integer().named('Service_ID').references(Services, #serviceId, onDelete: KeyAction.cascade)();
...
class KhorosServices extends Table {
  ...
  IntColumn get khorosId => integer().named('Khoros_ID').references(Khoroses, #khorosId, onDelete: KeyAction.cascade)();
  IntColumn get serviceId => integer().named('Service_ID').references(Services, #serviceId, onDelete: KeyAction.cascade)();
...
class StageServices extends Table {
  ...
  IntColumn get stageId => integer().named('Stage_ID').references(Stages, #stageId, onDelete: KeyAction.cascade)();
  IntColumn get serviceId => integer().named('Service_ID').references(Services, #serviceId, onDelete: KeyAction.cascade)();
...
class UserPermissionsExt extends Table {
  ...
  IntColumn get userId => integer().named('User_ID').references(Pass, #passId, onDelete: KeyAction.cascade)();
  TextColumn get featureKey => text().named('Feature_Key')(); // e.g., 'persons', 'reports'
  BoolColumn get canAdd => boolean().withDefault(const Constant(false)).named('can_add')();
  BoolColumn get canEdit => boolean().withDefault(const Constant(false)).named('can_edit')();
  BoolColumn get canDelete => boolean().withDefault(const Constant(false)).named('can_delete')();
...
class UserVisibilityFilters extends Table {
  ...
  IntColumn get userId => integer().named('User_ID').references(Pass, #passId, onDelete: KeyAction.cascade)();
  TextColumn get filterType => text().named('Filter_Type')(); // e.g., 'stage', 'service', 'khoros', 'area', 'father'
...
class Stages extends Table {
  ...
  TextColumn get stageName => text().nullable().named('Stage_Name')();
  IntColumn get serviceId => integer().nullable().named('Service_ID').references(Services, #serviceId, onDelete: KeyAction.setNull)();
  IntColumn get nextStageId => integer().nullable().named('Next_Stage_ID').references(Stages, #stageId, onDelete: KeyAction.setNull)();
}
...
class CustomFieldDefinitions extends Table {
  ...
  TextColumn get fieldKey => text().unique().nullable().named('Field_Key')(); // null for custom, or 'stage' etc.
  TextColumn get name => text().named('Name')();
  TextColumn get type => text().named('Type')(); // 'text', 'dropdown', 'multi_select', 'native'
  TextColumn get options => text().nullable().named('Options')(); // JSON array
  IntColumn get fieldOrder => integer().named('Field_Order')();
  BoolColumn get isVisible => boolean().withDefault(const Constant(true)).named('Is_Visible')();
  BoolColumn get isFilter => boolean().withDefault(const Constant(false)).named('Is_Filter')();
}

@DataClassName('PersonCustomFieldValue')
class PersonCustomFieldValues extends Table {
  IntColumn get personId => integer().named('Person_ID').references(Persons, #personId, onDelete: KeyAction.cascade)();
  IntColumn get fieldId => integer().named('Field_ID').references(CustomFieldDefinitions, #id, onDelete: KeyAction.cascade)();
...
class FamilyRelationships extends Table {
  ...
  IntColumn get relatedPersonId => integer().named('Related_Person_ID').references(Persons, #personId, onDelete: KeyAction.cascade)();
